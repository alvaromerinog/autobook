  ...
  ProviderContainer makeContainer() {
    final container = ProviderContainer(
      overrides: [
        ...
        createCarUseCaseProvider
            .overrideWithValue(CreateCarUseCase(mockRepo, mockIds)),
        syncPendingCarsUseCaseProvider
            .overrideWithValue(SyncPendingCarsUseCase(mockRepo)),
      ],
    );
  ...
  group('CarList', () {
    ...
    group('add', () {
      test(
        ...
        'when add is called, '
        'then state is updated with the new car list and no error',
        () async {
          // given
          when(() => mockRepo.refreshFromRemote()).thenAnswer((_) async {});
          when(() => mockRepo.syncPending()).thenAnswer((_) async {});
    ...
    group('syncPendingCars via coordinator flush', () {
      test(
        'given pending cars exist, '
        'when the coordinator flushes, '
        'then syncPendingCars runs and state is updated',
        () async {
          // given
          when(() => mockRepo.refreshFromRemote()).thenAnswer((_) async {});
          when(() => mockRepo.syncPending()).thenAnswer((_) async {});
